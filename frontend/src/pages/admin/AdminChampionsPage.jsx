...
import PageTitle from '../../components/PageTitle'; import { EmptyState, ErrorMessage, Loading } from '../../components/Status'
...
export default function AdminChampionsPage() {
  ...
  useEffect(()=>{getLeagues().then(data=>{setLeagues(data);if(data.length)setLeagueId(String(data[0].id))}).catch(()=>{setError('데이터를 불러오지 못했습니다. 잠시 후 다시 시도해주세요.');setLeagues([])})},[])
  useEffect(()=>{if(!leagueId)return;Promise.all([getLeagueTeams(leagueId),getLeagueWinner(leagueId)]).then(([teamRows,winnerData])=>{setTeams(teamRows);setWinner(winnerData.winner);setTeamId(winnerData.winner?String(winnerData.winner.teamId):'')}).catch(()=>{setError('우승팀 정보를 불러오지 못했습니다. 잠시 후 다시 시도해주세요.');setTeams([])})},[leagueId])
  if(leagues===null)return <><PageTitle title="우승팀 관리" back/><Loading/></>
  if(leagues.length===0)return <><PageTitle eyebrow="ADMIN · CHAMPION" title="우승팀 관리"/><>{error?<ErrorMessage text={error}/>:<EmptyState text="등록된 리그가 없습니다."/>}</></>
