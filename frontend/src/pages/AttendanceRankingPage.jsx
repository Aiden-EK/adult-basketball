...
function RankingGroup({ title, type, participants, totalAttendanceDays }) {
  ...
  return <section className="attendance-ranking-group">
    <div className="attendance-ranking-heading"><h2 className={isGuest ? 'attendance-ranking-guest' : ''}>{title}</h2><span>{ranking.length}명</span></div>
    {ranking.length === 0
      ? <div className="empty card attendance-ranking-empty"><p className="muted">등록된 {title}이 없습니다.</p></div>
      : <div className="attendance-ranking-list card">{ranking.map(participant => <article className="attendance-ranking-row" key={participant.leagueMemberId}>
