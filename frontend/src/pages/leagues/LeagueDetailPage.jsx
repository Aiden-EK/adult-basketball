...
const tabs = [['standings', '팀 순위'], ['scorers', '개인 득점'], ['games', '경기'], ['participants', '참가자'], ['winner', '우승팀']]
...
function StandingsTable({ standings }) {
  ...
  return <div className="standings-card card"><div className="standings-scroll"><table className="standings-table"><thead><tr><th>순위</th><th>팀명</th><th>경기</th><th>승</th><th>패</th><th>득점</th><th>실점</th><th>득실차</th><th>승률</th></tr></thead><tbody>{standings.map(team => <tr key={team.teamId}><td><b className="rank-number">{team.rank}</b></td><th scope="row">{team.teamName}</th><td>{team.gamesPlayed}</td><td>{team.wins}</td><td>{team.losses}</td><td>{team.pointsFor}</td><td>{team.pointsAgainst}</td><td className={team.pointDifference > 0 ? 'positive' : team.pointDifference < 0 ? 'negative' : ''}>{team.pointDifference > 0 ? '+' : ''}{team.pointDifference}</td><td><b>{formatWinRate(team.winRate)}</b></td></tr>)}</tbody></table></div></div>
}
...
export default function LeagueDetailPage() {
  const { id } = useParams(); const [league, setLeague] = useState(null); const [standings, setStandings] = useState(null); const [scorers, setScorers] = useState(null); const [winner, setWinner] = useState(undefined); const [participants, setParticipants] = useState(null); const [teams, setTeams] = useState([]); const [games, setGames] = useState(null); const [scores, setScores] = useState({}); const [tab, setTab] = useState('standings'); const [error, setError] = useState('')
