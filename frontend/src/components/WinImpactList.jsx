...
function TeamDetail({ team }) {
  return <div className="win-impact-team"><div><b>{team.teamName}</b><span>{team.games}경기 · {team.wins}승 {team.losses}패</span></div><div><span>참가 {percent(team.winRate)}</span><span>팀 평균 {percent(team.teamAverageWinRate)}</span><strong className={team.winImpact > 0 ? 'positive' : team.winImpact < 0 ? 'negative' : ''}>{impact(team.winImpact)}</strong></div></div>
}
