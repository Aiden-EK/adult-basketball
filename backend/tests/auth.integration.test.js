...
async function main() {
  ...
  try {
    ...
    assert.equal((await request('/admin/members', { headers: { Cookie: cookie } })).status, 200); assert.equal((await request('/admin/members')).status, 401); assert.equal((await request('/members')).status, 401); assert.equal((await request('/leagues')).status, 200);
