...
function validate(input) {
  ...
  if (input.status === 'SCHEDULED' && input.resultType !== null) return '예정 경기에는 경기 결과를 입력할 수 없습니다.';
  if (input.status === 'COMPLETED' && !['NORMAL', 'TIEBREAK', 'FORFEIT'].includes(input.resultType)) return '유효하지 않은 경기 결과 유형입니다.';
...
const fields = `g.id AS "gameId", gd.league_id AS "leagueId", gd.game_date AS "gameDate", g.game_no AS "gameNo", g.scheduled_at AS "scheduledAt", g.status, g.team_a_id AS "homeTeamId", home.name AS "homeTeamName", g.team_b_id AS "awayTeamId", away.name AS "awayTeamName", g.team_a_score AS "homeScore", g.team_b_score AS "awayScore", g.result_type AS "resultType", g.winner_team_id AS "winnerTeamId", g.created_at AS "createdAt", g.updated_at AS "updatedAt"`;
...
router.patch('/:gameId', async (req, res) => {
  ...
  try {
    ...
    await pool.query('UPDATE game SET team_a_id=$1, team_b_id=$2, team_a_score=$3, team_b_score=$4, status=$5, result_type=$6, winner_team_id=$7, scheduled_at=NULL, updated_at=CURRENT_TIMESTAMP WHERE id=$8', [homeTeamId, awayTeamId, homeScore, awayScore, status, resultType, winnerTeamId, gameId]);
