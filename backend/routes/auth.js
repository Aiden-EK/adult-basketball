...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    res.setHeader('Set-Cookie', serializeCookie(COOKIE_NAME, token, cookieOptions({ secure: req.secure })));
...
router.post('/logout', async (req, res) => {
  try {
    ...
    res.setHeader('Set-Cookie', serializeCookie(COOKIE_NAME, '', { ...cookieOptions({ secure: req.secure }), maxAge: 0 }));
