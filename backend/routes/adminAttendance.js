...
function validAttendanceItem(item) {
  if (!id(item?.leagueMemberId) || !['PRESENT', 'ABSENT'].includes(item?.status)) return false;
  if (item.status === 'PRESENT') return Boolean(id(item.actualTeamId));
  return item.actualTeamId === null;
}
...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    const teams = await pool.query(`SELECT id, name, sort_order AS "sortOrder" FROM team WHERE league_id=$1 ORDER BY sort_order, id`, [leagueId]);
    const members = await pool.query(`SELECT lm.id AS "leagueMemberId", m.id AS "memberId", m.name, m.grade AS "membershipType", lm.team_id AS "teamId", t.name AS "teamName", a.status AS "attendanceStatus", a.actual_team_id AS "actualTeamId"
    ...
    res.json({ leagueId, attendanceDate, games: games.rows, teams: teams.rows, members: members.rows });
...
router.put('/:attendanceDate', async (req, res) => {
  ...
  if (!leagueId || !attendanceDate || !Array.isArray(attendance) || attendance.some(item => !validAttendanceItem(item))) return res.status(400).json({ message: '참석자는 출전팀을 선택하고, 불참자는 출전팀을 비워주세요.' });
  ...
  try {
    ...
    for (const item of attendance) {
      ...
      if (item.status === 'PRESENT' && !(await client.query('SELECT 1 FROM team WHERE id=$1 AND league_id=$2', [item.actualTeamId, leagueId])).rowCount) {
        await client.query('ROLLBACK');
        return res.status(400).json({ message: '현재 리그에 속한 출전팀만 선택할 수 있습니다.' });
      }
...
module.exports.validAttendanceItem = validAttendanceItem;
